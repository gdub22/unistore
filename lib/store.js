...
class Store {
  ...
  _findByKey (type, key, opts) {
    const cached = this._getByKey(type, key)
    return cached && Promise.resolve(cached) || this._fetchByKey(type, key, opts)
  }

  _findMany (type, params, opts) {
    const cached = this._getMany(type, params)
    if (cached && cached.length) {
      return Promise.resolve(cached)
    }
    return this._fetchMany(type, params, opts)
  }

  _fetchByKey (type, key, opts) {
    return this.adapter.fetch(type, key, opts).then(data => this._pushOne(type, data))
  }

  _fetchMany (type, params, opts) {
    return this.adapter.fetch(type, params, opts).then(data => this._pushMany(type, data))
  }
}
